...
pub(crate) fn upload(
    ...
    let auth = (!api_key.is_empty()).then(|| api_key.clone());
    ...
    eprintln!("[comppics] POST /comparison -> {status}");
    ...
    for_each_cell(st, params, positions, overlays, |row, col, buf| {
        ...
        eprintln!("[comppics] POST /comparison/{id}/image ({row},{col}) -> {status}");
        if !status.is_success() {
    ...
    eprintln!("[comppics] comparison complete -> https://comp.pics/compare/{id}");
