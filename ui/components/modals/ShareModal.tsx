...
import { CircleCheckBig, Copy, ExternalLink, GripVertical, HelpCircle, Loader2 } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(async () => {
      ...
      try {
        ...
        console.log("[tags] autofillTags", JSON.stringify(name), "->", auto.length, auto);
      } catch (e) {
      ...
      >
        ...
        {provider === "comppics" ? (
          ...
          <Field label="Cookie">
            <Input
              type="password"
              value={appSettings.slowpicsCookie}
              onChange={(e) => void saveAppSettings({ ...appSettings, slowpicsCookie: e.target.value })}
              placeholder="Optional"
